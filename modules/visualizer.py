...
def generate_station_popup_html(row, df_anual_melted, include_chart=False,
    ...
    # Intenta obtener el rango de años de la sesión, si no existe usa un valor por defecto
    year_min = st.session_state.get('year_range', [2000])[0]
    year_max = st.session_state.get('year_range', [2020])[1]
...
def create_enso_chart(enso_data):
    ...
    # Trazas de Fondo (Barra de Fases)
    ...
    # Trazas de Leyenda (Trazas fantasma)
    ...
    # Traza de la Anomalía ONI (Línea principal)
    fig.add_trace(go.Scatter(
...
def display_graphs_tab(df_anual_melted, df_monthly_filtered, stations_for_analysis, gdf_filtered):
    ...
    st.info(f"Mostrando análisis para {selected_stations_str} en el período "
            f"{st.session_state.year_range[0]} - {st.session_state.year_range[1]} y meses seleccionados.")
...
def display_advanced_maps_tab(gdf_filtered, stations_for_analysis, df_anual_melted, df_monthly_filtered):
    ...
    try:
        if method == "Kriging Ordinario":
            ...
            # --- INICIO DE LA CORRECCIÓN DEL VARIOGRAMA ---
            # Se crea la figura de Matplotlib manualmente en lugar de usar ok.display_variogram_model()
            ...
            ax.legend()
            # --- FIN DE LA CORRECCIÓN DEL VARIOGRAMA ---
            ...
            # La interpolación RBF espera que grid_lon y grid_lat formen una malla
            grid_x, grid_y = np.meshgrid(grid_lon, grid_lat)
            z_grid_raw = rbf(grid_x, grid_y)
            z_grid = z_grid_raw # No es necesario transponer con meshgrid
    ...
    if z_grid is not None:
        fig = go.Figure(data=go.Contour(z=z_grid.T, x=grid_lon, y=grid_lat, # Transponemos aquí para el contour plot
    ...
    with indices_climaticos_tab:
        ...
        if not available_indices:
            ...
            if selected_index and selected_station_corr:
                # Mapear la etiqueta seleccionada al nombre de columna en Config
        ...
        if not available_tabs:
            ...
            for i, var_name in enumerate(available_tabs):
                with enso_variable_tabs[i]:
                    ...
                    enso_filtered = df_enso[(df_enso[Config.DATE_COL].dt.year >=
                                             st.session_state.year_range[0]) &
            ...
            with st.spinner(f"Entrenando modelo Prophet y generando pronóstico para {station_to_forecast_prophet}..."):
                try:
                    ...
                    st.error(f"Ocurrió un error al generar el pronóstico con Prophet. Esto puede deberse a "
                             f"que la serie de datos es demasiado corta o inestable. Error: {e}")
            ...
            if not df_station.empty:
                ...
                try:
                    ...
                    st.error(f"No se pudo realizar la descomposición de la serie para la estación "
                             f"seleccionada. Es posible que la serie de datos sea demasiado corta o no tenga una estructura "
                             f"estacional clara. Error: {e}")
            else:
            ...
            if not df_station_acf.empty:
                ...
                if len(df_station_acf) > max_lag:
                    ...
                    st.warning(f"No hay suficientes datos (se requieren > {max_lag} meses) para la estación "
                               f"{station_to_analyze_acf} para realizar el análisis de autocorrelación.")
            else:
