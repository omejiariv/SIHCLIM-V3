...
from modules.utils import display_plotly_download_buttons, add_folium_download_button
# <--- CAMBIO CLAVE: Importamos calculate_spi y calculate_monthly_anomalies desde analysis.py
        ...
        with mensual_graf_tab:
            if not df_monthly_filtered.empty:
                ...
                if chart_type != "Gráfico de Cajas (Distribución Mensual)":
                    ...
                    st.altair_chart(final_chart.properties(title=f"Serie de Precipitación Mensual ({st.session_state.year_range[0]} - {st.session_state.year_range[1]})").interactive(),
    ...
    with anim_tab:
        ...
        if not df_anual_melted.empty:
            ...
            if all_years:
                ...
                # CORRECCIÓN DE PALETA PLOTLY: Usando YIGnBu (estándar de Plotly)
            ...
            st.warning("No hay datos anuales para el mapa animado.")
            ...
            def generate_interpolation_map(year, method, gdf_filtered_map):
                ...
                try:
                    if method == "Kriging Ordinario":
                        ...
                        # Nota: La función Rbf devuelve una función, que luego se evalúa en la grilla 2D.
                        grid_x, grid_y = np.meshgrid(grid_lon, grid_lat)
                        z_grid_raw = rbf(grid_x.flatten(), grid_y.flatten()).reshape(grid_x.shape)
                        z_grid = z_grid_raw.T  # Transponer para Plotly
        ...
        if station_to_analyze_spi:
            ...
            if len(precip_series.dropna()) < spi_window * 2:
                ...
                with st.spinner(f"Calculando SPI-{spi_window}..."):
                    # <--- CAMBIO CLAVE: Usamos la función de analysis.py
...
def display_anomalies_tab(df_long, df_monthly_filtered, stations_for_analysis):
    ...
    # <--- CAMBIO CLAVE: Usamos la función de analysis.py para obtener anomalías
    ...
    with anom_extremos_tab:
        ...
        with col1:
            ...
            # Filtrar por las columnas necesarias. La columna 'precip_promedio_mes' fue calculada en calculate_monthly_anomalies
        ...
        with col2:
            ...
            st.dataframe(humedos.rename(columns={Config.STATION_NAME_COL: 'Estación',
                                                 'anomalia': 'Anomalía (mm)', Config.PRECIPITATION_COL: 'Ppt. (mm)', 'precip_promedio_mes': 'Ppt. Media (mm)'}).round(0), use_container_width=True)
...
def display_trends_and_forecast_tab(df_anual_melted, df_monthly_to_process,
    ...
    with mann_kendall_tab:
        ...
        with st.expander("¿Qué es la prueba de Mann-Kendall?"):
            st.markdown("""
            La **Prueba de Mann-Kendall** es un método estadístico no paramétrico utilizado para detectar tendencias en series de tiempo.
            ...
            **Pendiente de Sen (Sen's Slope)**: Es un método para cuantificar la magnitud de la tendencia, calculando la mediana de todas las pendientes entre pares de puntos.
